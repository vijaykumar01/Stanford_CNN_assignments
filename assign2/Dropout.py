...
# In the file `cs231n/layers.py`, implement the backward pass for dropout. After doing so, run the following cell to
...
# Specificially, if the constructor the the net receives a nonzero value for the `dropout` parameter, then the net
# should add dropout immediately after every ReLU non-linearity. After doing so, run the following to numerically
...
for dropout in [0, 0.25, 0.5]:
  ...
  model = FullyConnectedNet([H1, H2], input_dim=D, num_classes=C,
                            ...
                            dropout=dropout, use_batchnorm=False,seed=123)
...
# and one will use a dropout probability of 0.75. We will then visualize the training and validation accuracies of the
#  two networks over time.
...
for dropout in dropout_choices:
  model = FullyConnectedNet([500], dropout=dropout, use_batchnorm=False)
  print dropout
